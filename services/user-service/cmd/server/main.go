...
import (
	userservice "github.com/Khalid-Abdullahi-Isse/social-media-backend/services/user-service"
	"github.com/Khalid-Abdullahi-Isse/social-media-backend/services/user-service/internal/config"
...
func main() {
	...
	limiter, err := ratelimit.New(rateRedis, "user-service", rateConfig.Timeout)
	if err != nil {
		log.Fatal(err)
	}
	...
	defer sqlDB.Close()
	controller := userservice.BuildController(db)
	router := httpcontroller.NewRouter(controller, func(r *gin.Engine) {
		if err := ratelimit.Install(r, limiter, rateConfig, "user-service"); err != nil {
			log.Fatal(err)
		}
	})
